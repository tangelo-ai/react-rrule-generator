...
const SimpleRender = () => <RRuleGenerator onChange={(rrule) => console.log(`RRule changed, now it's ${rrule}`)} />;
...
class ControlledRender extends Component {
  ...
  render() {
    return <RRuleGenerator onChange={(rrule) => this.setState({ rrule })} value={this.state.rrule} />;
  }
}
```
...
- Switch to working node version: `nvm use 18`
